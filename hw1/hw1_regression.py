...
class HW1:
  def train(self,x_train, y_train, epoch):
    ...
    for i in range(epoch):
      ...
      for j in range(3200):
        ...
        for k in range(9):
          w_g += (y_train[j] - weights.dot(x_train[j, 9, :]) - bias) * x_train[j, 9, k] * (-1)
      b_g /= 3200
      w_g /= 3200

      for i in range(9):
        w_g[i] += reg_rate * weights[i]
  ...
  def vaildation(self,x_val, y_val, weights, bias):
    loss = 0
    for i in range(400):
  ...
  def __init__(self):
    ...
    epoch = 2000
    w, b = self.train(x_train, y_train, epoch)
    print(w,b)
